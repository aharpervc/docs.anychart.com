...
To add range marker you need to use {api:anychart.standalones.axisMarkers.Range}rangeMarker(){api} method. By default this method creates a marker on a primary y axis. You can bind the marker to another axis using {api:anychart.core.axisMarkers.Range#axis}axis(){api} method. The basic range marker's properties can be set using {api:anychart.standalones.axisMarkers.Range#from}from(){api}, {api:anychart.standalones.axisMarkers.Range#to}to(){api} and {api:anychart.standalones.axisMarkers.Range#fill}fill(){api} methods.
