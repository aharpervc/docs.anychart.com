...
As you see, we can define some parameters of each linemarker such as color, dashes or thickness using {api:anychart.standalones.axisMarkers.Line#stroke}.stroke(){api} method. It is used absolutely the same way as any other line settings with the
{api:anychart.standalones.axisMarkers.Line#stroke}.stroke(){api} method.
...
* Any type of Gantt diagram will return "false" for {api:anychart.standalones.axisMarkers.Line#isHorizontal}.isHorizontal(){api} method, because markers in Gantts can be only vertical.
